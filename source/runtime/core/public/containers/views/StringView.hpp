...
#include <ranges>     // For std::ranges algorithms and views
...
/// @tparam CharT The character type of the string view. Must satisfy the IsCharacter concept.
...
public:
    ...
    GP_NODISCARD constexpr SizeType find(BasicStringView sv, SizeType pos = 0) const noexcept
    {
        ...
        auto it = std::ranges::search(begin() + pos, end(), sv.begin(), sv.end());
        return it.empty() ? npos : static_cast<SizeType>(it.begin() - begin());
    }
    ...
    GP_NODISCARD constexpr SizeType find(CharT ch, SizeType pos = 0) const noexcept
    {
        ...
        auto it = std::ranges::find(begin() + pos, end(), ch);
    ...
    GP_NODISCARD constexpr SizeType rfind(BasicStringView sv, SizeType pos = npos) const noexcept
    {
        if (sv.isEmpty())
        {
            return gp::math::min(pos, m_size);
        }
        if (sv.m_size > m_size)
        {
            return npos;
        }
        pos = gp::math::min(pos, m_size - sv.m_size);
        auto sub = std::ranges::subrange(begin(), begin() + pos + sv.m_size);
        auto it = std::ranges::find_end(sub.begin(), sub.end(), sv.begin(), sv.end());
        return it.empty() ? npos : static_cast<SizeType>(it.begin() - begin());
    }
    ...
    GP_NODISCARD constexpr SizeType rfind(CharT ch, SizeType pos = npos) const noexcept
    {
        ...
        auto it = std::ranges::find(std::views::reverse(sub), ch);
        return it != std::views::reverse(sub).end() ? static_cast<SizeType>(it.base() - begin() - 1) : npos;
    }
    ...
    GP_NODISCARD constexpr SizeType findFirstOf(BasicStringView sv, SizeType pos = 0) const noexcept
    {
        ...
        auto it = std::ranges::find_first_of(begin() + pos, end(), sv.begin(), sv.end());
        return it != end() ? static_cast<SizeType>(it - begin()) : npos;
    }
    ...
    GP_NODISCARD constexpr SizeType findLastOf(BasicStringView sv, SizeType pos = npos) const noexcept
    {
        ...
        pos = gp::math::min(pos, m_size - 1);
        auto sub = std::ranges::subrange(begin(), begin() + pos + 1);
        auto it = std::ranges::find_first_of(std::views::reverse(sub), sv);
        return it != std::views::reverse(sub).end() ? static_cast<SizeType>(it.base() - begin() - 1) : npos;
    }
    ...
    GP_NODISCARD constexpr SizeType findFirstNotOf(BasicStringView sv, SizeType pos = 0) const noexcept
    {
        ...
        auto it = std::ranges::find_if_not(
            ...
            [&sv](CharT c)
        {
            return std::ranges::contains(sv, c);
        }
        );
        return it != end() ? static_cast<SizeType>(it - begin()) : npos;
    }
...
/// @brief Non-owning, read-only `char` view into a contiguous character sequence.
using StringView = container::BasicStringView<char>;

/// @brief Non-owning, read-only `wchar_t` view into a contiguous character sequence.
using WStringView = container::BasicStringView<wchar_t>;

/// @brief Non-owning, read-only `char8_t` view into a contiguous character sequence.
using U8StringView = container::BasicStringView<char8_t>;

/// @brief Non-owning, read-only `char16_t` view into a contiguous character sequence.
using U16StringView = container::BasicStringView<char16_t>;

/// @brief Non-owning, read-only `char32_t` view into a contiguous character sequence.
using U32StringView = container::BasicStringView<char32_t>;
...
template <gp::concepts::IsCharacter CharT>
struct std::formatter<gp::container::BasicStringView<CharT>, CharT>
    : std::formatter<std::basic_string_view<CharT>, CharT>
{
    template <typename FormatContext>
    auto format(gp::container::BasicStringView<CharT> sv, FormatContext& ctx) const
    {
