...
#include "platform/PlatformMemoryDefines.hpp"
#include <cstring>
...
{
public:
    /// @brief Indicates whether the system is currently in an out of memory condition.
    static bool isOutOfMemory;

    /// @brief The size of the allocation that caused the out of memory condition.
    static gp::UInt64 outOfMemoryAllocationSize;

    /// @brief The alignment used for out of memory allocations.
    static gp::UInt32 outOfMemoryAllocationAlignment;

    /// @brief A backup memory pool used when the system runs out of memory. Thiqs is used by Out Of Memory handling and
    /// crash reporting to ensure that there is some memory available to report the OOM condition and capture a crash
    /// dump.
    static void* backupOutOfMemoryMemoryPool;

    /// @brief The size of the backup memory pool used when the system runs out of memory.
    static gp::UInt32 backupOutOfMemoryMemoryPoolSize;

    /// @brief The actual memory allocator type used by this platform.
    static MemoryAllocatorType allocatorType;
