...
/// \brief Base implementation of the IWindow interface providing state-tracked, platform-agnostic window logic.
/// All member state is managed in this class, making it directly usable as a headless window when no display
/// context is available (e.g., server-side rendering, unit tests, or offscreen pipelines). Platform-specific
/// subclasses override individual methods to add OS-level operations while invoking the base to synchronize state.
class FBaseWindow : public IWindow
{
protected:
    static constexpr UInt32 DEFAULT_WIDTH = 1280;
    static constexpr UInt32 DEFAULT_HEIGHT = 720;
    static constexpr Float32 DEFAULT_OPACITY = 1.0f;
    static constexpr FIntExtent2D DEFAULT_MIN_SIZE{ 100, 100 };
    static constexpr FIntExtent2D DEFAULT_MAX_SIZE{ 0, 0 };
    static constexpr Char8 DEFAULT_TITLE[]{ "Untitled Window" };
    static constexpr EWindowMode DEFAULT_WINDOW_MODE = EWindowMode::Windowed;
    static constexpr EWindowState DEFAULT_WINDOW_STATE = EWindowState::Normal;
    static constexpr EWindowStyle DEFAULT_WINDOW_STYLE = EWindowStyle::Default;
    static constexpr ECursorMode DEFAULT_CURSOR_MODE = ECursorMode::Normal;
    static constexpr ECursorShape DEFAULT_CURSOR_SHAPE = ECursorShape::Arrow;
    static constexpr FIntPoint2D DEFAULT_POSITION{ 0, 0 };
    static constexpr Float32 DEFAULT_ASPECT_RATIO =
        static_cast<Float32>(DEFAULT_WIDTH) / static_cast<Float32>(DEFAULT_HEIGHT);
    static constexpr Float32 DEFAULT_CONTENT_SCALE = 1.0f;

protected:
    EWindowMode m_windowMode{ DEFAULT_WINDOW_MODE };
    EWindowState m_windowState{ DEFAULT_WINDOW_STATE };
    EWindowStyle m_windowStyle{ DEFAULT_WINDOW_STYLE };
    ECursorMode m_cursorMode{ DEFAULT_CURSOR_MODE };
    ECursorShape m_cursorShape{ DEFAULT_CURSOR_SHAPE };
    Float32 m_opacity{ DEFAULT_OPACITY };
    FString m_title{ DEFAULT_TITLE };
    FIntExtent2D m_minSize{ DEFAULT_MIN_SIZE };
    FIntExtent2D m_maxSize{ DEFAULT_MAX_SIZE };
    FIntExtent2D m_fullWindowSize{ DEFAULT_WIDTH, DEFAULT_HEIGHT };
    FIntExtent2D m_contentSize{ DEFAULT_WIDTH, DEFAULT_HEIGHT };
    FIntExtent2D m_framebufferSize{ DEFAULT_WIDTH, DEFAULT_HEIGHT };
...
public:
    /// \brief Constructs a base window in windowed or headless mode.
    /// \note In headless mode the window is considered open immediately with no associated display surface and a null
    /// native handle. This is suitable for server-side, offscreen, or unit-test contexts.
    /// \param headless When true the window starts in an open state with no display requirements.
    explicit FBaseWindow(bool headless = false) noexcept;
