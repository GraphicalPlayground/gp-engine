...
/// \brief SHA-512 (Secure Hash Algorithm 512) implementation per FIPS 180-4.
/// Produces a 512-bit (64-byte) message digest. All operations are constexpr-capable.
struct SHA512 final : public Template::NonInstantiable
{
public:
    static constexpr SizeT BLOCK_SIZE = 128;   //<! SHA-512 processes 128-byte blocks.
    static constexpr SizeT DIGEST_SIZE = 64;   //<! SHA-512 produces a 64-byte digest.

private:
    /// \brief Initial hash values (FIPS 180-4 Section 5.3.5).
    static constexpr UInt64 H0 = 0x6A09E667F3BCC908ULL;
    static constexpr UInt64 H1 = 0xBB67AE8584CAA73BULL;
    static constexpr UInt64 H2 = 0x3C6EF372FE94F82BULL;
    static constexpr UInt64 H3 = 0xA54FF53A5F1D36F1ULL;
    static constexpr UInt64 H4 = 0x510E527FADE682D1ULL;
    static constexpr UInt64 H5 = 0x9B05688C2B3E6C1FULL;
    static constexpr UInt64 H6 = 0x1F83D9ABFB41BD6BULL;
    static constexpr UInt64 H7 = 0x5BE0CD19137E2179ULL;

    /// \brief Round constants K (FIPS 180-4 Section 4.2.3).
    ...
    /// \brief SHA-512 big Sigma1: ROTR(14) ^ ROTR(18) ^ ROTR(41).
    ...
    /// \brief SHA-512 small sigma0: ROTR(1) ^ ROTR(8) ^ SHR(7).
    GP_NODISCARD static constexpr UInt64 sigma0(UInt64 x) noexcept
    {
        return Detail::HashUtilities::Rotr64(x, 1) ^ Detail::HashUtilities::Rotr64(x, 8) ^ (x >> 7);
    }

    /// \brief SHA-512 small sigma1: ROTR(19) ^ ROTR(61) ^ SHR(6).
    GP_NODISCARD static constexpr UInt64 sigma1(UInt64 x) noexcept
    {
...
public:
    /// \brief Compute SHA-512 digest for a data buffer.
    /// \param data Pointer to the input bytes.
    /// \param length Number of bytes to hash.
    /// \return SHA-512 digest (64 bytes).
    GP_NODISCARD static constexpr SHA512Digest Hash(const char* data, SizeT length) noexcept
    {
        ...
        // SHA-512 appends a 128-bit length. Upper 64 bits are zero for messages < 2^64 bits.
    ...
    /// \brief Compute SHA-512 digest from a void pointer.
    /// \param data Pointer to the input bytes.
    /// \param length Number of bytes to hash.
    /// \return SHA-512 digest.
    GP_NODISCARD static constexpr SHA512Digest Hash(const void* data, SizeT length) noexcept
    {
        return Hash(static_cast<const char*>(data), length);
    }

    /// \brief Compute SHA-512 digest for a null-terminated C-string.
    /// \param str Null-terminated input string.
    /// \return SHA-512 digest.
    GP_NODISCARD static constexpr SHA512Digest Hash(const char* str) noexcept
    {
        return Hash(str, std::char_traits<char>::length(str));
    }

    /// \brief Compute SHA-512 digest from a string view.
    /// \param view String view to hash.
    /// \return SHA-512 digest.
    GP_NODISCARD static constexpr SHA512Digest Hash(std::string_view view) noexcept
    {
        return Hash(view.data(), view.size());
    }

    /// \brief Verify data against an expected SHA-512 digest.
    /// \param data Pointer to the input bytes.
    ...
    GP_NODISCARD static constexpr bool Verify(const char* data, SizeT length, const SHA512Digest& expected) noexcept
    {
    ...
    /// \param expected Expected digest value.
    /// \return True if computed digest matches expected.
    GP_NODISCARD static constexpr bool Verify(const void* data, SizeT length, const SHA512Digest& expected) noexcept
    {
        return Hash(data, length) == expected;
    }

    /// \brief Streaming SHA-512 context for incremental hashing.
    struct Context
    {
    public:
        UInt64 state[8] = { H0, H1, H2, H3, H4, H5, H6, H7 };
    ...
    public:
        ...
        /// \return SHA-512 digest.
        GP_NODISCARD constexpr SHA512Digest Finalize() noexcept
        {
            char padded[256]{};
