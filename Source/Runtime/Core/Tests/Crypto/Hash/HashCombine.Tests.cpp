...
TEST_CASE("HashCombine Utilities", "[GP][Core][Crypto][Hash][HashCombine]")
{
    ...
    SECTION("Combine32 Order Matters")
    {
        auto hash1 = HashCombine::Combine32(100U, 200U);
        auto hash2 = HashCombine::Combine32(200U, 100U);
        REQUIRE(hash1 != hash2);
    }
    ...
    SECTION("Combine64 Order Matters")
    {
        auto hash1 = HashCombine::Combine64(100ULL, 200ULL);
        auto hash2 = HashCombine::Combine64(200ULL, 100ULL);
        REQUIRE(hash1 != hash2);
    }

    SECTION("Combine32 and Combine64 Are Independent")
    {
        auto h32 = static_cast<GP::UInt64>(HashCombine::Combine32(0U, 1U));
        auto h64 = HashCombine::Combine64(0ULL, 1ULL);
        // Different golden-ratio constants; results must differ.
        REQUIRE(h32 != h64);
    }
    ...
    SECTION("GoldenRatio Constant Accessible")
    {
#if GP_ARCHITECTURE_64BIT
        REQUIRE(HashCombine::GoldenRatio == 0x9E3779B97F4A7C15ULL);
        REQUIRE(sizeof(HashCombine::GoldenRatio) == 8);
#else
        REQUIRE(HashCombine::GoldenRatio == 0x9E3779B9U);
        REQUIRE(sizeof(HashCombine::GoldenRatio) == 4);
#endif
    }

    SECTION("GoldenRatio Contributes to Mixing")
    {
        // seed=0, value=0: with golden ratio the result must be non-trivial.
        auto hash = HashCombine::Combine(0, 0);
        REQUIRE(hash != 0);
    }
    ...
    SECTION("Combine32 Deterministic")
    {
        auto h1 = HashCombine::Combine32(0xDEADBEEFU, 0x12345678U);
        auto h2 = HashCombine::Combine32(0xDEADBEEFU, 0x12345678U);
        REQUIRE(h1 == h2);
    }

    SECTION("Combine64 Deterministic")
    {
        auto h1 = HashCombine::Combine64(0xDEADBEEFDEADBEEFULL, 0x0123456789ABCDEFULL);
        auto h2 = HashCombine::Combine64(0xDEADBEEFDEADBEEFULL, 0x0123456789ABCDEFULL);
        REQUIRE(h1 == h2);
    }
    ...
    SECTION("Compile-time Evaluation")
    {
        ...
        constexpr auto hash3 = HashCombine::Combine32(42U, 99U);
        constexpr auto hash4 = HashCombine::Combine64(42ULL, 99ULL);
        REQUIRE(hash1 != 0);
        REQUIRE(hash2 != 0);
        REQUIRE(hash3 != 0);
        REQUIRE(hash4 != 0);
...
TEST_CASE("HashCombine Edge Cases", "[GP][Core][Crypto][Hash][HashCombine]")
{
    using namespace GP::Crypto;
