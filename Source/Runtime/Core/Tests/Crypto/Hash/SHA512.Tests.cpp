...
TEST_CASE("SHA512 Hashing", "[GP][Core][Crypto][Hash][SHA512]")
{
    ...
    SECTION("FIPS 180-4 Empty String Vector")
    {
        ...
        constexpr UInt8 kExpected[64] = { 0xCF, 0x83, 0xE1, 0x35, 0x7E, 0xEF, 0xB8, 0xBD, 0xF1, 0x54, 0x28, 0x50, 0xD6,
    ...
    SECTION("FIPS 180-4 'abc' Vector")
    {
        ...
        constexpr UInt8 kExpected[64] = { 0xDD, 0xAF, 0x35, 0xA1, 0x93, 0x61, 0x7A, 0xBA, 0xCC, 0x41, 0x73, 0x49, 0xAE,
    ...
    SECTION("SHA512 Differs from SHA384 on Same Input")
    {
        ...
        REQUIRE(static_cast<UInt8>(d512.bytes[0]) == 0xDD);    // first byte of known SHA-512("abc")
        REQUIRE(static_cast<UInt8>(d512.bytes[63]) == 0x9F);   // last byte
    }

    SECTION("Compile-Time Evaluation")
    {
        constexpr auto d = SHA512::Hash("abc", 3);
        static_assert(static_cast<UInt8>(d.bytes[0]) == 0xDD, "SHA512 compile-time evaluation failed");
        REQUIRE(static_cast<UInt8>(d.bytes[0]) == 0xDD);
        REQUIRE(static_cast<UInt8>(d.bytes[63]) == 0x9F);
    }
}
