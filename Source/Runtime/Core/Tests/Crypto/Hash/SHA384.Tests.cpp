...
TEST_CASE("SHA384 Hashing", "[GP][Core][Crypto][Hash][SHA384]")
{
    ...
    SECTION("FIPS 180-4 Empty String Vector")
    {
        ...
        constexpr UInt8 kExpected[48] = { 0x38, 0xB0, 0x60, 0xA7, 0x51, 0xAC, 0x96, 0x38, 0x4C, 0xD9, 0x32, 0x7E,
    ...
    SECTION("FIPS 180-4 'abc' Vector")
    {
        ...
        constexpr UInt8 kExpected[48] = { 0xCB, 0x00, 0x75, 0x3F, 0x45, 0xA3, 0x5E, 0x8B, 0xB5, 0xA0, 0x3D, 0x69,
    ...
    SECTION("SHA384 Differs from SHA512 on Same Input")
    {
        ...
        REQUIRE(static_cast<UInt8>(d384.bytes[0]) == 0xCB);    // first byte of known SHA-384("abc")
        REQUIRE(static_cast<UInt8>(d384.bytes[47]) == 0xA7);   // last byte
    }

    SECTION("Compile-Time Evaluation")
    {
        constexpr auto d = SHA384::Hash("abc", 3);
        static_assert(static_cast<UInt8>(d.bytes[0]) == 0xCB, "SHA384 compile-time evaluation failed");
        REQUIRE(static_cast<UInt8>(d.bytes[0]) == 0xCB);
        REQUIRE(static_cast<UInt8>(d.bytes[47]) == 0xA7);
    }
}
